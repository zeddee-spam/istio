...
func newLocalController(t *testing.T) (*Controller, *FakeXdsUpdater) {
	fx := NewFakeXDS()
	ki := makeClient(t)
	ctl := NewController(ki, nil, ControllerOptions{
		WatchedNamespaces: "",
		ResyncPeriod:      resync,
...
type ControllerOptionsMutatorFunc func(options *ControllerOptions)
...
func TestController_getPodAZ_FromPod(t *testing.T) {
	...
	for _, c := range testCases {
		t.Run(c.name, func(t *testing.T) {
			...
			// Setup kube caches
			controller, fx := newFakeController(t, func(o *ControllerOptions) {
